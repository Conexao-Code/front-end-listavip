...
export const ModalAntecipado = ({
  ...
  promoterToken,
  eventColor
}: ModalAntecipadoProps): JSX.Element | null => {
    ...
    const ticketPrice =
      genero === 'masculino'
        ...
        : genero === 'feminino'
          ...
          : '';
    try {
      const response = await fetch(`${API_URL}/api/check-participation-by-cpf`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ cpf: cpf, eventName }),
      });
      if (!response.ok) {
        const data = await response.json();
        if (response.status === 409) {
          toast.error(data.message || "O cliente já está registrado neste evento.");
        } else {
          toast.error(data.error || "Erro na verificação de participação.");
        }
        return;
      }
      onClose({
      ...
      toast.error("Erro ao verificar participação.");
    }
  };
        ...
        <div className="relative flex flex-col w-full bg-[#1A202C] rounded-2xl overflow-hidden shadow-2xl border border-gray-700 my-auto">
            ...
            <div className="space-y-4" style={{ "--event-color": eventColor } as React.CSSProperties}>
              <div className="relative">
                <div className="flex items-center gap-3 mb-2">
                  <div className="p-2 rounded-lg" style={{ backgroundColor: hexToRgba(eventColor, 0.1) }}>
                    <User className="w-5 h-5" style={{ color: eventColor }} />
                  </div>
                  <label className="text-gray-300 text-sm sm:text-base font-medium" htmlFor="nomeCompleto">
                    Nome Completo
                  </label>
                </div>
                <input
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="relative">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <label className="text-gray-300 text-sm sm:text-base font-medium" htmlFor="cpf">
                      CPF
                    </label>
                  </div>
                  <input
                ...
                <div className="relative">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <label className="text-gray-300 text-sm sm:text-base font-medium" htmlFor="whatsapp">
                      WhatsApp
                    </label>
                  </div>
                  <input
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="relative">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <label className="text-gray-300 text-sm sm:text-base font-medium" htmlFor="email">
                      E-mail
                    </label>
                  </div>
                  <input
                ...
                <div className="relative">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <label className="text-gray-300 text-sm sm:text-base font-medium" htmlFor="genero">
                      Gênero
                    </label>
                  </div>
                  <select
          ...
          <div className="flex flex-col sm:flex-row gap-3 p-4 sm:p-6 border-t border-gray-700">
            ...
            <button
              ...
              className="px-6 py-3 rounded-lg text-gray-900 hover:bg-[var(--event-color)] transition-colors font-medium flex-1 sm:flex-none"
