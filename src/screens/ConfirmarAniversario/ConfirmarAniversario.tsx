...
export const ConfirmarAniversario = (): JSX.Element => {
  ...
  const incrementGuestCount = async (ip: string) => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        localStorage.setItem("visitedTokens", JSON.stringify({ ...storedVisits, [token!]: ip }));
      } else {
  ...
  useEffect(() => {
    if (events.length > 0) {
      const birthdayName = events[0]?.birthday_name || "Evento";
      document.title = `Aniversário de ${birthdayName}`;
    } else {
      document.title = "Evento";
    }
  }, [events]);

  const handleModalConfirmarOpen = (
    eventName: string,
  ...
  const handleModalConfirmarClose = (
    data: { name: string; cpf: string; email: string; phone: string; gender: string; ticketPrice: string; qrCodeUrl?: string } | null
  ) => {
  ...
  const handleModalAntecipadoOpen = (
    eventName: string,
    ticketPriceMen: string,
    ticketPriceWomen: string,
    eventColor: string
  ...
  const handleModalAntecipadoClose = (
    data: { name: string; cpf: string; email: string; phone: string; gender: string; ticketPrice: string } | null
  ) => {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`relative w-full h-full min-h-screen flex flex-col items-center justify-center p-4 md:p-8 ${
            company?.blur_effect === "active" ? "backdrop-blur-[15px]" : ""
          }`}
        >
                ...
                <div className="hidden lg:block relative w-full max-w-[824px] h-[374px] rounded-2xl overflow-hidden mx-auto bg-white/5 backdrop-blur-sm shadow-2xl hover:shadow-3xl transition-all duration-300">
                  <div className="absolute inset-0">
                    <div className="relative h-full flex">
                      ...
                      <div className="w-[300px] p-6 flex flex-col" style={{ backgroundColor: event.event_color }}>
                        ...
                        <h2 className="text-2xl font-bold mb-6 text-center">{event.event_name}</h2>
                        {event.attractions && (
                          <div className="mb-6">
                ...
                <div className="lg:hidden w-full max-w-[400px] mx-auto">
                  ...
                  >
                    <div className="relative h-[280px]">
                      ...
                      {(() => {
                        const numButtons = (event.guest_list === "active" ? 1 : 0) + (event.event_batch === "active" ? 1 : 0);
      ...
      />
      <ModalTicket isOpen={isModalTicketOpen} onClose={handleModalTicketClose} ticketInfo={selectedTicketInfo} />
...
export default ConfirmarAniversario;
